...
export type OcrEngineId = "ocr-space" | "google-vision" | "tesseract";

const SVG_PLACEHOLDER = "(SVG는 OCR 대상이 아닙니다. 대체텍스트를 직접 입력해 주세요.)";

export async function requestOcrForImageItem(item: OcrImageItem, engine: OcrEngineId = "google-vision"): Promise<{ ok: true; text: string } | { ok: false; message: string }> {
